...
public class Questao04 {
    ...
    public boolean testar(String entrada) {
        ...
        for (int step = 1; step < cadeia.length; step++) {
            ...
            for (int j = step; j < cadeia.length; j++) {
                for (int k = i; k < j; k++) {
                    Set<String> combinacoes = combinarProducoes(tabela[i][k],tabela[k+1][j]);
    ...
    private HashSet<String> combinarProducoes(HashSet<String> producoesA, HashSet<String> producoesB){
        HashSet<String> combinacoes = new HashSet<String>();
        for (String itemProducaoA : producoesA)
            for (String itemProducaoB : producoesB)
